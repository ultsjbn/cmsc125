...
void *philosopher(void *arg) {
    ...
    /* Thread-safe random seed for this specific philosopher */
    ...
    for (int meal = 0; meal < MEALS_PER_PHILOSOPHER; meal++) {
        ...
        int think_time = (rand_r(&seed) % THINK_TIME_MAX) + 1;
        ...
        int eat_time = (rand_r(&seed) % EAT_TIME_MAX) + 1;
