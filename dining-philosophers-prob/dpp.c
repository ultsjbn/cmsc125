...
static void print_state_table(void) {
	...
	printf("  +");
	for (int i = 0; i < NUM_PHILOSOPHERS + 16; i++) printf("-");
	printf("+\n\n");
	pthread_mutex_unlock(&log_mutex);
}
...
/* ------------ Helper: Open Semaphores ------------ */
static void open_semaphores(void) {
	char name[32];
 
	room = sem_open(SEM_ROOM_NAME, O_CREAT | O_EXCL, 0644, NUM_PHILOSOPHERS - 1);
	if (room == SEM_FAILED) {
